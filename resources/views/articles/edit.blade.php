...
<div x-data="articleEditor()"
    ...
    <!-- Main Editor Area -->
    <form id="articleForm" action="{{ route('articles.update', $article) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6">
            ...
            <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                ...
                <p class="text-sm text-red-600">{{ $article->rejection_reason ?? 'Silakan perbaiki dan kirim ulang.' }}</p>
            </div>
            @endif
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-1 editor-main">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <textarea name="content" id="content" x-ref="editor">{{ old('content', $article->content) }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <label class="cursor-pointer">
                                <input type="radio" name="level" value="pemula" class="peer hidden" @checked(old('level', $article->level ?? 'pemula') === 'pemula')>
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="level" value="menengah" class="peer hidden" @checked(old('level', $article->level ?? '') === 'menengah')>
                                <span class="block text-center px-3 py-2 text-sm rounded-lg border-2 border-gray-200
                            ...
                            <label class="cursor-pointer">
                                <input type="radio" name="level" value="lanjut" class="peer hidden" @checked(old('level', $article->level ?? '') === 'lanjut')>
                                <span class="block text-center px-3 py-2 text-sm rounded-lg border-2 border-gray-200
