...
@section('title', 'Edit Artikel - IDN Menulis')
...
@section('styles')
    <style>
        ...
        .focus-mode:hover .sidebar-panel,
...
@section('content')
    <div x-data="articleEditor()" :class="{ 'focus-mode': focusMode }" @keydown.ctrl.s.prevent="saveDraft()"
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            <span :class="{
                                ...
                                'bg-orange-100 text-orange-700': status === 'revision'
        ...
        <!-- Main Editor Area -->
        <form id="articleForm" action="{{ route('articles.update', $article) }}" method="POST"
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6">
                ...
                @if($article->status === 'rejected')
                    <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl">
                        ...
                        <p class="text-sm text-red-600">{{ $article->rejection_reason ?? 'Silakan perbaiki dan kirim ulang.' }}
                        </p>
                    </div>
                @endif
                    ...
                    <div class="flex-1 editor-main">
                        ...
                        <div class="mb-6">
                            <div class="cover-upload relative rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 hover:border-primary-400 transition-colors cursor-pointer"
                                ...
                                @click="$refs.coverInput.click()" @dragover.prevent="isDragging = true"
                                @dragleave.prevent="isDragging = false" @drop.prevent="handleCoverDrop($event)">
                        ...
                        <div class="mb-6">
                            <input type="text" name="title" x-model="title"
                                ...
                                placeholder="Tulis Judul Artikel di Sini..." @input="updateStats()" required>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            <textarea name="content" id="content"
                                x-ref="editor">{{ old('content', $article->content) }}</textarea>
                        </div>
                    </div>
                    ...
                    <aside class="sidebar-panel w-full lg:w-80 space-y-6">
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                            ...
                            <div class="flex flex-wrap gap-2 max-h-40 overflow-y-auto custom-scrollbar">
                                @php $articleTags = $article->tags->pluck('id')->toArray(); @endphp
                                @foreach($tags as $tag)
                                    <label class="tag-pill cursor-pointer">
                                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="peer hidden"
                                            @checked(in_array($tag->id, old('tags', $articleTags)))
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <label class="cursor-pointer">
                                    <input type="radio" name="level" value="pemula" class="peer hidden"
                                        @checked(old('level', $article->level ?? 'pemula') === 'pemula')>
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="level" value="menengah" class="peer hidden"
                                        @checked(old('level', $article->level ?? '') === 'menengah')>
                                    <span class="block text-center px-3 py-2 text-sm rounded-lg border-2 border-gray-200
                                ...
                                <label class="cursor-pointer">
                                    <input type="radio" name="level" value="lanjut" class="peer hidden"
                                        @checked(old('level', $article->level ?? '') === 'lanjut')>
                                    <span class="block text-center px-3 py-2 text-sm rounded-lg border-2 border-gray-200
        ...
        <!-- Publish Modal -->
        <div x-show="showPublishModal" x-transition:enter="transition ease-out duration-200"
                    ...
                    <label class="block p-4 border-2 rounded-xl cursor-pointer transition-all"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                                ...
                                <div x-show="publishOption === 'schedule'" class="w-3 h-3 rounded-full bg-primary-500">
                                </div>
                            </div>
                            <div>
                    ...
                    <div x-show="publishOption === 'schedule'" x-transition class="pl-8 space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="date" x-model="scheduleDate" :min="minDate"
        ...
        <!-- Preview Modal -->
        <div x-show="showPreview" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" class="fixed inset-0 bg-black/50 z-50 overflow-y-auto"
            ...
            <div class="min-h-screen py-8 px-4">
                <div class="max-w-3xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div class="p-8">
                        ...
                        <!-- Title -->
                        <h1 class="text-4xl font-bold font-display text-gray-900 mb-4" x-text="title || 'Judul Artikel'">
                        </h1>
    ...
    <script>
        function articleEditor() {
            return {
                ...
                category: @json(old('category_id', $article->category_id)),
                status: @json($article->status),
                coverPreview: @json($coverUrl),
                isDragging: false,
                ...
                init() {
                    ...
                    // Initialize stats after a short delay
                ...
                initTinyMCE() {
                    tinymce.init({
                        ...
                        toolbar_sticky: true,
                        toolbar_sticky_offset: 65, // Height of the sticky header + 1px border
                ...
                async saveDraft(isAuto = false) {
                    ...
                    localStorage.setItem('article_edit_draft_{{ $article->id }}', JSON.stringify(draftData));
